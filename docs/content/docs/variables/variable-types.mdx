...
- **Response Body**: Use JSONPath syntax like `body.user.id` or `body.data[0].name`
- **Response Headers**: Use `headers.header-name` syntax like `headers.x-request-id`
- **Cookies**: Use `cookies.cookie-name` syntax like `cookies.sessionId`
- **Status**: Use `status` to capture the HTTP status code
- **Response Time**: Use `responseTime` to capture request duration
