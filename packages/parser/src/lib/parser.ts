import { Flow, FlowStep, HttpRequest, HttpMethod, Directive, CaptureDirective, AssertDirective } from '@restflow/types';
...
export function parseFlow(content: string): ParseResult {
  ...
  for (const section of sections) {
    try {
      ...
      errors.push(`Error parsing step: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
...
function parseRequest(lines: string[], startIndex: number): { httpRequest: HttpRequest; nextIndex: number } {
...
function parseDirectives(lines: string[]): Directive[] {
  ...
  for (const line of lines) {
    ...
    if (directiveContent.startsWith('capture ')) {
      const captureMatch = directiveContent.match(/^capture\s+(\w+)\s*=\s*(.+)$/);
      if (captureMatch) {
