...
export class Formatter {
	...
	createHeader(title: string): string {
		const coloredTitle = this.colors 
	 ...
	 */
	formatStepProgress(stepName: string, stepIndex: number, totalSteps: number): string {
		const icons = this.getIcons();
	...
	formatHttpResult(result: StepResult): string {
		...
		if (error) {
			...
			const method = this.colors ? pc.bold(step.request.method) : step.request.method;
		...
		if (response) {
			...
			const timing = this.colors ? pc.dim(`${icons.time} ${duration}ms`) : `${icons.time} ${duration}ms`;
	...
	formatDirectives(directives: any[]): string[] {
		...
		return directives.map(directive => {
			...
			const statusColored = this.colors 
				? (directive.success ? pc.green(status) : pc.red(status))
				: status;
				
			const type = this.colors ? pc.cyan(directive.directive.type) : directive.directive.type;
	...
	createSummaryTable(result: FlowResult): string {
		...
		const overallStatus = result.success 
			? (this.colors ? pc.green(`${icons.success} PASSED`) : `${icons.success} PASSED`)
			: (this.colors ? pc.red(`${icons.error} FAILED`) : `${icons.error} FAILED`);
	...
	createOverallSummaryTable(
		...
		const summary = this.colors ? pc.bold("Overall Summary:") : "Overall Summary:";
	...
	private getDirectiveDescription(directive: any): string {
		if (directive.type === "assert") {
			...
			const variable = this.colors ? pc.green(directive.variable) : directive.variable;
			const expression = this.colors ? pc.dim(directive.expression) : directive.expression;
